...
`ep`'s reshape calls `ep_gluPerspective(300.0, …)`. That `300` is the *original IRIS GL* `perspective()` convention — **tenths of a degree** (= 30°) — not the 300 degrees a literal `gluPerspective` reads. So the port maps it to IRIS `perspective(300, …)` directly; feeding 300 to the fork's real `gluPerspective` yields a Y-flipped frustum that IRIS `window()` clips away (a black screen).
