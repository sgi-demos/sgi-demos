...
// Set whenever a colormap entry changes (mapcolor) or the CI front buffer
// is replaced (swapbuffers); tells gl_resolve_ci_if_needed the front RGB
// cache is stale and must be re-derived from the CI buffer. Starts 1 so the
// first present resolves.
static int ci_rgb_cache_stale = 1;
...
void swapbuffers() {
    ...
    if (!rgb_mode)
        ci_rgb_cache_stale = 1; // new front CI buffer: its RGB cache is by definition stale
    gl_resolve_ci_if_needed();
    events_frame_complete();
}
