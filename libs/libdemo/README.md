...
Source is `sgi/lib/libdemo` from the IRIS 4D demo tape as archived at bitsavers (sgidemos.tar.Z; hash in the [source archives table](../../docs/COPYRIGHT.md#8-source-archives)); colormod.c and grey.c are byte-identical to the tape and gamma.c, getset.c, and demolib.h differ by a dozen lines of build fixes. `Makefile_orig` is SGI's. The files carry only their author lines, no copyright or license notice, and are used under fair use; see [docs/COPYRIGHT.md](../../docs/COPYRIGHT.md#3-categories-of-material-and-the-basis-for-using-each).
